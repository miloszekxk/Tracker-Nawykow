...
export default function HabitItem({ habit }: { habit: Habit }) {
  ...
  if(isEditing){
    return(
      <li className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex flex-col sm:flex-row gap-2">
  ...
  return (
    <li className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
      <div className="flex flex-col min-w-0">
        <span className="font-medium truncate">{habit.name}</span>
        <span className="text-sm text-slate-400 whitespace-nowrap">🔥 {habit.streak} dni</span>
      </div>
